...
# settings
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
model_thresh = 0.8                # >0.5 ..0.9  # Umbrales para la detección de artefactos.
n_components = 8                  # Número de componentes ICA y opciones de procesamiento paralelizado (usando GPU con 'cuda').
njobs = 'cuda'
# Parámetros de filtrado para las distintas etapas del procesamiento
sfreq_new = 200                           # downsampling to 250 Hz
...
# Define una ventana temporal fija para analizar un fragmento de la señal
nsamples_chop = 12001
...
# ----------------------------------------------
# get times to crop
dt = raw.times[1]             # time period between two time samples
tmin = ix_t1 * dt
...
ica.fit(raw, verbose=None)
sources = ica.get_sources(raw)._data                     # get sources
...
# ----------------------------------------------
# plot sources
fig = ica.plot_sources(raw, title='MLICA', show=False)
...
ica_filtered_chop = ica_update_mean_std(raw, ica, picks=picks, reject=reject)
raw_filtered_chop_clean = ica_filtered_chop.apply(raw, exclude=ica.exclude,
                                                  n_pca_components=None)
ica_unfiltered_chop = ica_update_mean_std(raw, ica, picks=picks, reject=reject)
raw_unfiltered_chop_clean = ica_unfiltered_chop.apply(raw, exclude=ica.exclude, n_pca_components=None)
